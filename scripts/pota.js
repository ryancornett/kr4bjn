...
function populateActivationCards() {
  const upcomingActivations = regionalActivations.filter(
    (act) => act.startDate >= todayStr
  );
  upcomingActivations.forEach((activation) => {
    ...
    infoRowText.textContent = activation.name;
    ...
    const stateRowRight = document.createElement("p");
    stateRowRight.textContent = states[activation.locationDesc];
    ...
    frequencyRowRight.textContent =
      activation.frequencies == "None" ||
      activation.frequencies == "" ||
      activation.frequencies == null
        ? "No frequency given"
        : activation.frequencies;
    ...
    utcRowRight.textContent = `${activation.startDate.substring(5)} ${
    ...
    cardFooter.textContent =
      activation.comments == "" || activation.comments == null
